...
class ParseStringArgumentsUseCaseTests(ParseArgumentsTestTemplate):
    ...
    def test_simple_string_present(self):
        argument_parser = ParseArgumentsUseCase([(ArgumentSchemaElement('x', '*'))], ['-x', 'param'], self.am_factory, self.present_help_message_use_case)
        self.assertTrue(argument_parser.has("x"))

    def test_long_string_name(self):
        argument_parser = ParseArgumentsUseCase(
            [ArgumentSchemaElement('x', '*', long_name='excelsior')], ['--excelsior', 'alpha'], self.am_factory, self.present_help_message_use_case)
    ...
    def test_extra_arguments(self):
        argument_parser = ParseArgumentsUseCase([(ArgumentSchemaElement('y', '*'))],
                                                ['-y', 'alpha', 'beta'], self.am_factory, self.present_help_message_use_case)
